...
export default function MentalHPEngine() {
  ...
  const generateShareImage = async (): Promise<File | null> => {
    ...
    try {
      ...
      if (!blob) return null;
      return new File([blob], "mental-hp-result.png", { type: "image/png" });
    } catch {
      el.style.cssText = orig;
      return null;
    }
  ...
  const shareWithImage = async (method: string) => {
    gtagEvent("share", { method, quiz_id: "mental-hp" });
    toast("이미지 생성 중...");
      ...
      if (window.Kakao && window.Kakao.isInitialized()) {
        window.Kakao.Share.sendDefault({
          ...
          content: {
            ...
            link: {
              mobileWebUrl: "https://pickmetype.vercel.app/mental-hp",
              webUrl: "https://pickmetype.vercel.app/mental-hp",
            },
          },
          buttons: [
            {
